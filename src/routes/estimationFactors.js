...
export default function createEstimationFactorsRouter(db) {
    ...
    const parseFactorRow = (row) => {
        ...
        row.valuePerResourceType = JSON.parse(row.value_per_resource_type || '{}');
        ...
        delete row.value_per_resource_type;
        return row;
    };
    ...
    router.post('/',
        ...
        async (req, res) => {
            ...
            const { name, description, hoursPerResourceType, valuePerResourceType, journal_entries } = req.body;

            // Check if value_per_resource_type column exists
            const tableInfo = await db.all(`PRAGMA table_info(estimation_factors)`);
            const hasValuePerResourceType = tableInfo.some(col => col.name === 'value_per_resource_type');
            ...
            const newDataForAudit = { 
                ...
                valuePerResourceType: valuePerResourceType || {}
            };
            const auditEntry = {
            ...
            if (hasValuePerResourceType) {
                await db.run(
                    'INSERT INTO estimation_factors (id, name, description, hours_per_resource_type, value_per_resource_type, journal_entries, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
                    [newId, name, description, JSON.stringify(hoursPerResourceType || {}), JSON.stringify(valuePerResourceType || {}), JSON.stringify(newJournalEntries), now, now]
                );
            } else {
    ...
    router.put('/:id',
        ...
        async (req, res) => {
            ...
            if (hasValuePerResourceType) {
                updateFields.value_per_resource_type = JSON.stringify(valuePerResourceType || {});
            }
            ...
            const oldDataForAudit = {
                ...
                valuePerResourceType: JSON.parse(oldFactor.value_per_resource_type || '{}'),
            };
            const newDataForAudit = { 
                name, 
                description, 
